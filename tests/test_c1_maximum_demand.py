...
class TestC1A1:
    def test_max_demand_1_to_20_points(self):
        load = C1A1(num=20, rating_w=10)
        assert load.get_maximum_demand_a() == 3

    def test_max_demand_21_points(self):
        load = C1A1(num=21, rating_w=10)
        assert load.get_maximum_demand_a() == 5

    def test_max_demand_40_points(self):
        load = C1A1(num=40, rating_w=10)
        assert load.get_maximum_demand_a() == 7
